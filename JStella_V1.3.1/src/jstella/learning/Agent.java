...
public class Agent {
	...
	//HashTables	
	HashMap<StateActionPair,  Double> prototypeAction = new HashMap<StateActionPair, Double>();
	HashMap<State, Integer> prototypeVisit = new HashMap<State, Integer>();
	...
	//  -______----------_--------_--------------------------
	//  -|-___-\--------|-|------|-|-------------------------
	//  -|-|_/-/-__-___-|-|_-___-|-|_-_---_-_-__---___--___--
	...
	private void createPrototypes(){
		...
		while(prototypeVisit.size() < numberOfPrototypes){
			...
			State prototype;
			if(rnd.nextInt(10)==1)
				prototype = new State(splitPrototype(JSI.getROMState()));
			else
				prototype = new State(createPrototype());
			...
			if(!prototypeVisit.containsKey(prototype)){
				...
				for(int[] a : validActions){
					...
					prototypeAction.put(new StateActionPair(prototype.getState(), a), 0.0);
				}
			}
		}
		System.out.println("-----");
	...
	private void updatePrototypes(){
		HashMap<State, Integer> newVisit = new HashMap<State, Integer>();
		HashMap<StateActionPair,  Double> newProtoAct = new HashMap<StateActionPair, Double>();

		//determine the probability for keeping a prototype p
		double probabilityToKeep; 
		double rndProb;
		for(State prototype : prototypeVisit.keySet()){
			probabilityToKeep = Math.pow(Math.E, -(prototypeVisit.get(prototype)));
			rndProb = rnd.nextDouble();
			//System.out.println("PRob:" + probabilityToKeep + "\nRand: " + r);
			if(rndProb > probabilityToKeep){//Keep the prototype
				newVisit.put(prototype, prototypeVisit.get(prototype));
			}

		}
		//Replace the visited table with the new one, ie. prototypes we kept
		prototypeVisit.clear();
		prototypeVisit = newVisit;

		for(StateActionPair sap : prototypeAction.keySet()){
			if(prototypeVisit.containsKey(new State(sap.getState()))){
				newProtoAct.put(sap, prototypeAction.get(sap));
			}
		}
		//Replace the state,action table with only those prototypes that survived
		prototypeAction.clear();
		...
		if(prototypeVisit.size() < (numberOfPrototypes/100)){//RANDOM
			...
			int[][] validActions = JSI.getROMValidActions();
			while(prototypeVisit.size() < numberOfPrototypes){
				State prototype = new State(createPrototype());
				if(!prototypeVisit.containsKey(prototype)){
					//Add this prototype to the visited hashtable 
					prototypeVisit.put(prototype, 0);
					for(int[] a : validActions)
			...
			int[][] validActions = JSI.getROMValidActions();
			while(prototypeVisit.size() < numberOfPrototypes){
				State prototype = new State(splitPrototype((int[])(prototypeVisit.keySet().toArray())[rnd.nextInt(prototypeVisit.size())]));
				if(!prototypeVisit.containsKey(prototype)){
					//Add this prototype to the visited hashtable 
					prototypeVisit.put(prototype, 0);
					for(int[] a : validActions)
	 ...
	 * @return
	 */
	private int[] splitPrototype(int[] prevState){//BUG FIX THIS EMTHODS
		...
		int memSpacePosition = 0;
		int numberOfValues = 0;
		while(numberOfValues<=1){
			variablePosition = rnd.nextInt(newState.length);
			memSpacePosition = indexOf(memspace[variablePosition], newState[variablePosition]);
			numberOfValues = memspace[variablePosition].length;
		}
