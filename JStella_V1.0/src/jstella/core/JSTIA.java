...
public class JSTIA implements IfcDevice, java.io.Serializable {
	...
	public int getCOLUBK(){
		return myTIAPokeRegister[COLUBK];  
	}

	public void setCOLUBK(int aValue){
		myTIAPokeRegister[COLUBK]=aValue; 
	}

	public int getCOLUPF(){ 
		return myTIAPokeRegister[COLUPF];   
	}

	public void setCOLUPF(int aValue){ 
		myTIAPokeRegister[COLUPF]=aValue; 
	}

	public int getCOLUP0(){ 
		return myTIAPokeRegister[COLUP0];  
	}

	public void setCOLUP0(int aValue){ 
		myTIAPokeRegister[COLUP0]=aValue;    
	}

	public int getCOLUP1(){ 
		return myTIAPokeRegister[COLUP1];   
	}

	public void setCOLUP1(int aValue){ 
		myTIAPokeRegister[COLUP1]=aValue;     
	}

	public int[] getCurrentFrameBuffer(){
		return myConsole.getVideo().getCurrentFrameBuffer(); 
	}
	...
	protected int scanlines() {
		return (int)((mySystem.getCycles() * CLOCKS_PER_CPU_CYCLE) - myClockWhenFrameStarted)/CLOCKS_PER_LINE_TOTAL;
	}
	...
	private JSAudio getAudio(){
		return myConsole.getAudio(); 
	}

	private int getCurrentClockCount() {
		return mySystem.getCycles() * CLOCKS_PER_CPU_CYCLE;    
	}

	private int getCurrentXPos() { 
		return ((getCurrentClockCount() - myClockWhenFrameStarted) % CLOCKS_PER_LINE_TOTAL); 
	}
	...
	private boolean isPlayer0PixelOn(int aHPos) {
		return ((myCurrentGRP0 & getCurrentP0Mask(aHPos))!=0);  
	}  

	private boolean isPlayer1PixelOn(int aHPos) {
		return ((myCurrentGRP1 & getCurrentP1Mask(aHPos))!=0);  
	} 

	private boolean isMissile0PixelOn(int aHPos) {
		return getCurrentM0Mask(aHPos);  
	}  

	private boolean isMissile1PixelOn(int aHPos) {
		return getCurrentM1Mask(aHPos);   
	}  

	private boolean isRESMP0()  {
		return ((myTIAPokeRegister[RESMP0] & BIT1) != 0);  
	}  

	private boolean isRESMP1()  {
		return ((myTIAPokeRegister[RESMP1] & BIT1) != 0); 
	}
