...
public class JSConsole implements java.io.Serializable {
	...
	//Ian Code
	private JSILearning myLearning=null;
	private int myLearningSpeed = 1;
	private int myLearningTimer = 0;
	//Ian Code
	...
	public void setMyLearningSpeed(int learnSpeed){
		myLearningSpeed = learnSpeed;
	}

	public int getMyLearningSpeed(){
		return myLearningSpeed;
	}
		...
		{   
			if (getTelevisionMode()==TELEVISION_MODE_GAME)
			{
				if (myCart!=null) 
				{
					...
					//Ian Code
					if(myLearningSpeed > 0){
						myLearningTimer++;
						if(myLearningTimer - myLearningSpeed == 0){
							myLearning.doLearning();
							myLearningTimer = 0;
						}
					}
					//Ian Code
