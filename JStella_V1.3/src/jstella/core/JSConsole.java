...
public class JSConsole implements java.io.Serializable {
	...
	//Ian Code
	private JSILearning myAgentInterface=null;
	private int myAgentCallDelay = 1;
	private int myAgentTimer = 0;
	//Ian Code
	...
	//Ian Code
	public void setJSILearning(JSILearning J){
		myAgentInterface = J;
	}

	public void setMyAgentCallDelay(int learnSpeed){
		myAgentCallDelay = learnSpeed;
	}

	public int getMyAgentCallDelay(){
		return myAgentCallDelay;
	}
	...
	public void setConsoleClient(IfcConsoleClient aConsoleClient) {
	...
	public JSController getController(Jack jack) {
		return (jack == Jack.LEFT) ? myControllers[0] : myControllers[1];
	}
	...
	public jstella.core.JSSystem getSystem()  { 
		return mySystem; 
	}
		...
		{   
			if (getTelevisionMode()==TELEVISION_MODE_GAME)
			{
				if (myCart!=null) 
				{
					...
					//Ian Code
					if(myAgentCallDelay > 0){
						myAgentTimer++;
						if(myAgentTimer - myAgentCallDelay == 0){
							myAgentInterface.doLearning();
							myAgentTimer = 0;
						}
					}
					//Ian Code
